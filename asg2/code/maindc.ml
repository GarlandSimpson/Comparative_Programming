...
let load_store_list = [Load '='; Store ('a', 3.1415926535897932384626433832795029);
                       Load 'a'; Store ('m', 2.7182818284590452353602874713526625);
					   Load 'm'; Load '?' ]

let sym_table = amake 256 (false, Bigint.zero)					   
					   
let exc_load_store = function 
    | Load sym -> (
	    let el = aget sym_table(ord sym) in match el with
		    | false, _ -> printf "register '%c' has nothing\n%!" sym
			| true, value -> printf "%c = %g\n%!" sym value
			)
	| Store (sym, value)->(
	    aset sym_table(ord sym) (true, value);
		printf "%c := %g\n%!" sym value
...
let rec print_number' number i = match(number, i) with
    ...
    | car :: crd, _ -> printf "%c" car; print_number' cdr ( i + 1)
